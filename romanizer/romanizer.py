    ...
    def romanize(self):
        ...
        for char in pronounced:
            if (re.match(hangul, char)):
                s = Syllable(char)
                #try:
                if s.initial == NULL_CONSONANT:
                    _romanized.append(onset[s.initial] + vowel[s.medial].capitalize() + coda[s.final])
                else:
                    _romanized.append(onset[s.initial].capitalize() + vowel[s.medial] + coda[s.final])
